...
function compileButton() {
  ...
  const gconf = Qcert.buildConfig(config);
  const query = document.getElementById("query").value;
  const queryInput = {
    'gconf' : gconf,
		'query' : query,
  };
  const compilationResult = Qcert.compile(queryInput)
  const compiledQuery = compilationResult.result;
...
function verify(result, expected) {
  ...
  for (const i = 0; i < result.length; i++) {
    const resultMember = result[i];
    const expectedMember = expected[i];
...
function clearButton() {
  ...
  if (document.getElementById("execresult")) {
    document.getElementById("execresult").innerHTML = "";
  }
  if (document.getElementById("allresults")) {
...
function addOption(sel, lang) {
  ...
  const t = document.createTextNode(lang);
  opt.appendChild(t);
  sel.appendChild(opt);
}
function displayAllResults(allresults) {
  if (document.getElementById("allresults")) {
    document.getElementById("allresults").innerHTML = "";
    for (const i = 0; i < allresults.length; i++) {
...
function addResult(file, lang, value) {
  if (document.getElementById("allresults")) {
    const allres = document.getElementById("allresults");
    const l = document.createElement("b");
    const t1 = document.createTextNode(lang);
    l.appendChild(t1);
    const query = document.createElement("pre");
    const t2 = document.createTextNode(value);
  ...
  if (files.length > 0) {
    ...
    if (file.name.endsWith(".sem")) {
      reader.onload = function(event) {
        const contents = btoa(String.fromCharCode.apply(null,
      ...
      reader.onload = function(event) {
        const contents = escapeHtml(event.target.result);
...
function isSQLSchema(schemaText) {
  ...
  const create = schemaText.search(/create/i);
  if (create < 0)
    return false;
  const brace = schemaText.indexOf('{');
  ...
  const balance = schemaText.substring(create + 6).trim();
  /* The next word must be 'table' (case insensitive) */
  const table = balance.search(/table/i);
  return table == 0;
}
function convertSQLSchema(toConvert) {
  ...
  const result = preProcess(toConvert, "sqlSchema2JSON", process);
}
function handleCSVs(files) {
  ...
  function readFile(index) {
    ...
    const reader = new FileReader();  
    reader.onload = function(event) {
      const typeName = file.name.endsWith(".csv") ? file.name.substring(0, file.name.length - 4) : file.name;
...
function completeCSVs(readFiles) {
  ...
  const toSend = JSON.stringify({schema: schema, delimiter: delimiter, data: readFiles});
  const process = function(result) {
    document.getElementById("input").innerHTML = result;
  }
  const result = preProcess(toSend, "csv2JSON", process);
}
