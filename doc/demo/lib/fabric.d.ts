// There is currently a bug (IPath should be Path) in the npm @types/fabric package
// and the `moveCursor:string` element of fabric.Object
// so for now, we include a modified copy here.  If the bug ever gets fixed,
// we should change to using it like a normal npm dependency
...
interface PathOptions extends IObjectOptions {
	/**
...
export interface Path extends Object, PathOptions {}
export class Path {
	/**
	 * Constructor
	...
	initialize(path?: any[], options?: PathOptions): Path;
	 ...
	 * @param [options] Options object
	 */
	static fromElement(element: SVGElement, callback: (path: Path) => any, options?: PathOptions): void;
	/**
