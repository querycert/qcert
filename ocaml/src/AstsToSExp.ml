...
let rec sexp_to_sqlpp_query (se : sexp) =
  begin match se with
  | STerm ("query", sfw) ->
      sexp_to_sfw sfw
  | STerm ("union", [STerm ("distinct",[]);q1;q2]) ->
      QSQLPP.sqlpp_sqlpp_union SPDistinct (sexp_to_sqlpp_query q1) (sexp_to_sqlpp_query q2)
  | STerm ("intersect", [STerm ("distinct",[]);q1;q2]) ->
      QSQLPP.sqlpp_sqlpp_intersect SPDistinct (sexp_to_sqlpp_query q1) (sexp_to_sqlpp_query q2)
  | STerm ("except", [STerm ("distinct",[]);q1;q2]) ->
      QSQLPP.sqlpp_sqlpp_except SPDistinct (sexp_to_sqlpp_query q1) (sexp_to_sqlpp_query q2)
  | STerm ("union", [q1;q2]) ->
      QSQLPP.sqlpp_sqlpp_union SPAll (sexp_to_sqlpp_query q1) (sexp_to_sqlpp_query q2)
  | STerm ("intersect", [q1;q2]) ->
      QSQLPP.sqlpp_sqlpp_intersect SPAll (sexp_to_sqlpp_query q1) (sexp_to_sqlpp_query q2)
  | STerm ("except", [q1;q2]) ->
      QSQLPP.sqlpp_sqlpp_except SPAll (sexp_to_sqlpp_query q1) (sexp_to_sqlpp_query q2)
  | STerm (sterm, _) ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ query: " ^ sterm))
  | _ ->
      raise (Qcert_Error "Not well-formed S-expr inside SQL++ query")
...
and sexp_to_sfw sfw =
  ...
  | (STerm ("select", selects)) :: (STerm ("from", [froms])) :: other_clauses ->
      let (where,group_by,order_by) = sexp_to_other_clauses other_clauses in
      QSQLPP.sqlpp_sqlpp_query
	(sexp_to_sqlpp_selects selects)
	(sexp_to_sqlpp_froms froms)
	where group_by order_by
  | (STerm ("union", [STerm ("distinct",[]);q1;q2])) :: [] ->
      QSQLPP.sqlpp_sqlpp_union SPDistinct (sexp_to_sqlpp_query q1) (sexp_to_sqlpp_query q2)
  | (STerm ("intersect", [STerm ("distinct",[]);q1;q2])) :: [] ->
...
and sexp_to_sqlpp_selects selects =
  begin match selects with
  | [] -> []
  | (STerm ("all",[])) :: selects' ->
      (QSQLPP.sqlpp_select_star
       :: (sexp_to_sqlpp_selects selects'))
  | (STerm ("as",[cname])) :: expr :: selects' ->
      (QSQLPP.sqlpp_select_expr (sstring_to_coq_string cname) (sexp_to_sqlpp_expr expr))
      :: (sexp_to_sqlpp_selects selects')
  | STerm ("deref",[cname;STerm ("ref",[tname])]) :: selects' ->
      (QSQLPP.sqlpp_select_column_deref (sstring_to_coq_string tname) (sstring_to_coq_string cname))
      :: (sexp_to_sqlpp_selects selects')
  | STerm ("ref",[cname]) :: selects' ->
      (QSQLPP.sqlpp_select_column (sstring_to_coq_string cname))
      :: (sexp_to_sqlpp_selects selects')
  | (STerm ("function", _) as expr) :: selects'
  | (STerm ("multiply", _) as expr) :: selects'
  | (STerm ("divide", _) as expr) :: selects' ->
      (QSQLPP.sqlpp_select_expr (Util.char_list_of_string "") (sexp_to_sqlpp_expr expr))
      :: (sexp_to_sqlpp_selects selects')
  | STerm (sterm, _) :: _ ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ select: " ^ sterm))
  | _ ->
...
and sexp_to_sqlpp_froms froms =
  begin match froms with
  | STerm ("table",[tname]) -> [QSQLPP.sqlpp_from_table (sstring_to_coq_string tname)]
  | STerm ("aliasAs", [new_tname;STerm ("table",[tname])]) ->
      [QSQLPP.sqlpp_from_table_alias (sstring_to_coq_string new_tname)
	 (sstring_to_coq_string tname)]
  | STerm ("aliasAs", [tname;query]) ->
      [QSQLPP.sqlpp_from_query (sstring_to_coq_string tname, None) (sexp_to_sqlpp_query query)]
  | STerm ("join", [from1;from2]) ->
      (sexp_to_sqlpp_froms from1) @ (sexp_to_sqlpp_froms from2)
  | STerm (sterm, _) ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ froms: " ^ sterm))
  | _ ->
      raise (Qcert_Error "Not well-formed S-expr inside SQL++ froms")
...
and sexp_to_other_clauses other_clauses =
  ...
  | [] -> (None, None, None)
  | STerm ("where",[cond]) :: rest ->
      let (group_by,order_by) = sexp_to_sqlpp_group_by_order_by rest in
      (Some (sexp_to_sqlpp_cond cond), group_by, order_by)
  | rest ->
      let (group_by,order_by) = sexp_to_sqlpp_group_by_order_by rest in
      (None, group_by, order_by)
  end
and sexp_to_sqlpp_group_by_order_by rest =
  begin match rest with
  | [] -> (None,None)
  | (STerm ("groupBy",groups)) :: (STerm ("having",[cond])) :: rest ->
      let order_by = sexp_to_sqlpp_order_by rest in
      let group_by = Some (sexp_to_sqlpp_groups groups, Some (sexp_to_sqlpp_cond cond)) in
      (group_by, order_by)
  | STerm ("groupBy",groups) :: rest ->
      let order_by = sexp_to_sqlpp_order_by rest in
      let group_by = Some (sexp_to_sqlpp_groups groups, None) in
      (group_by, order_by)
  | rest ->
      let order_by = sexp_to_sqlpp_order_by rest in
      (None, order_by)
  end
and sexp_to_sqlpp_order_by rest =
  begin match rest with
  | [] -> None
  | STerm ("orderBy",orders) :: [] ->
      Some (sexp_to_sqlpp_orders orders)
  | STerm ("orderBy",orders) :: STerm (sterm, _) :: _ ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ other clauses (after orderBy): " ^ sterm))
  | STerm (sterm, _) :: _ ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ other clauses: " ^ sterm))
...
and sexp_to_sqlpp_groups groups =
  begin match groups with
  | [] -> []
  | (STerm ("ref",[cname])) :: groups' -> (sstring_to_coq_string cname) :: sexp_to_sqlpp_groups groups'
  | STerm (sterm, _) :: _ ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ groups: " ^ sterm))
  | _ ->
      raise (Qcert_Error "Not well-formed S-expr inside SQL++ groups")
  end
and sexp_to_sqlpp_orders orders =
  begin match orders with
  | [] -> []
  | (STerm ("ascending",[(STerm ("ref",[cname]))])) :: orders' ->
      (sstring_to_coq_string cname, Ascending) :: sexp_to_sqlpp_orders orders'
  | (STerm ("descending",[(STerm ("ref",[cname]))])) :: orders' ->
      (sstring_to_coq_string cname, Descending) :: sexp_to_sqlpp_orders orders'
  | (STerm ("ascending",STerm (sterm,_)::_)) :: orders' ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ ascending order: " ^ sterm))
  | (STerm ("descending",STerm (sterm,_)::_)) :: orders' ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ descending order: " ^ sterm))
...
and sexp_to_sqlpp_expr expr =
  begin match expr with
  | SString s ->
      QSQLPP.sqlpp_expr_const (Dstring (char_list_of_string s))
  | SFloat f ->
      QSQLPP.sqlpp_expr_const (Dforeign (Obj.magic (Enhancedfloat f)))
  | SInt i ->
      QSQLPP.sqlpp_expr_const (Dnat i)
  | STerm ("dunit",[]) ->
      QSQLPP.sqlpp_expr_const (Dunit)
  | STerm ("list",const_list) ->
      QSQLPP.sqlpp_expr_const (Dcoll (sexp_to_sqlpp_const_list const_list))
(*  | STerm ("const_list",const_list) ->
      List.fold_left (QSQLPP.sqlpp_binary (map (fun e -> (QSQLPP.sqlpp_unary sexp_to_sqlpp_expr const_list)) *)
  | STerm ("cast",[STerm ("as",[SString "DATE"]); expr1]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Compiler.Enhanced_unary_sql_date_op Compiler.Uop_sql_date_from_string)))
	(sexp_to_sqlpp_expr expr1)
  | STerm ("literal",[SString "date"; SString sdate]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Enhanced_unary_sql_date_op Uop_sql_date_from_string)))
	(QSQLPP.sqlpp_expr_const (Dstring (char_list_of_string sdate)))
  | STerm ("interval",[SString sinterval; STerm ("year",[])]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Enhanced_unary_sql_date_op Uop_sql_date_interval_from_string)))
	(QSQLPP.sqlpp_expr_const (Dstring (char_list_of_string (sinterval ^ "-YEAR"))))
  | STerm ("interval",[SString sinterval; STerm ("month",[])]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Enhanced_unary_sql_date_op Uop_sql_date_interval_from_string)))
	(QSQLPP.sqlpp_expr_const (Dstring (char_list_of_string (sinterval ^ "-MONTH"))))
  | STerm ("interval",[SString sinterval; STerm ("day",[])]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Enhanced_unary_sql_date_op Uop_sql_date_interval_from_string)))
	(QSQLPP.sqlpp_expr_const (Dstring (char_list_of_string (sinterval ^ "-DAY"))))
  | STerm ("deref",[cname;STerm ("ref",[tname])]) ->
      QSQLPP.sqlpp_expr_column_deref (sstring_to_coq_string tname) (sstring_to_coq_string cname)
  | STerm ("ref",[cname]) -> (QSQLPP.sqlpp_expr_column (sstring_to_coq_string cname))
  | STerm ("minus",[expr1]) ->
      QSQLPP.sqlpp_expr_unary SPMinus (sexp_to_sqlpp_expr expr1)
  | STerm ("add",[expr1;expr2]) ->
      QSQLPP.sqlpp_expr_binary SPPlus (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("subtract",[expr1;expr2]) ->
      QSQLPP.sqlpp_expr_binary SPSubtract (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("multiply",[expr1;expr2]) ->
      QSQLPP.sqlpp_expr_binary SPMult (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("divide",[expr1;expr2]) ->
      QSQLPP.sqlpp_expr_binary SPDivide (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "substr";expr1;SInt n1;SInt n2]) ->
     QSQLPP.sqlpp_expr_unary (SPSubstring (n1-1,Some (n1-1+n2))) (sexp_to_sqlpp_expr expr1) (* It's 'substring (expr from n1 for n2)' i.e., from n1 for n2 characters, with initial index 1 *)
  | STerm ("cases",
	   [STerm ("case", [STerm ("when", [condexpr])
			  ; STerm ("then", [expr1])])
	   ; STerm ("default", [expr2])]) ->
     QSQLPP.sqlpp_expr_case
       (sexp_to_sqlpp_cond condexpr)
       (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "count"]) ->
      QSQLPP.sqlpp_expr_agg_expr SPCount QSQLPP.sqlpp_expr_star
  | STerm ("function",[SString "count";expr1]) ->
      QSQLPP.sqlpp_expr_agg_expr SPCount (sexp_to_sqlpp_expr expr1)
  | STerm ("function",[SString "sum";expr1]) ->
      QSQLPP.sqlpp_expr_agg_expr SPSum (sexp_to_sqlpp_expr expr1)
  | STerm ("function",[SString "avg";expr1]) ->
      QSQLPP.sqlpp_expr_agg_expr SPAvg (sexp_to_sqlpp_expr expr1)
  | STerm ("function",[SString "min";expr1]) ->
      QSQLPP.sqlpp_expr_agg_expr SPMin (sexp_to_sqlpp_expr expr1)
  | STerm ("function",[SString "max";expr1]) ->
      QSQLPP.sqlpp_expr_agg_expr SPMax (sexp_to_sqlpp_expr expr1)
  | STerm ("query", _) -> QSQLPP.sqlpp_expr_query (sexp_to_sqlpp_query expr) (* XXX Nested query XXX *)
  | STerm ("extract",[STerm ("year",[]);expr1]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Enhanced_unary_sql_date_op (Uop_sql_get_date_component Sql_date_YEAR))))
	(sexp_to_sqlpp_expr expr1)
  | STerm ("extract",[STerm ("month",[]);expr1]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Enhanced_unary_sql_date_op (Uop_sql_get_date_component Sql_date_MONTH))))
	(sexp_to_sqlpp_expr expr1)
  | STerm ("extract",[STerm ("day",[]);expr1]) ->
      QSQLPP.sqlpp_expr_unary (SPUnaryForeignExpr (Obj.magic (Enhanced_unary_sql_date_op (Uop_sql_get_date_component Sql_date_DAY))))
	(sexp_to_sqlpp_expr expr1)
  | STerm ("function",[SString "date_minus";expr1;expr2]) ->
      QSQLPP.sqlpp_expr_binary (SPBinaryForeignExpr (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_minus)))
	(sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "date_plus";expr1;expr2]) ->
      QSQLPP.sqlpp_expr_binary (SPBinaryForeignExpr (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_plus)))
	(sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "concat";expr1;expr2]) ->
      QSQLPP.sqlpp_expr_binary SPConcat
	(sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",(STerm ("partitionBy",_))::(SString fun_name)::_) ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ expr: function (using 'partition by') " ^ fun_name))
  | STerm ("function", (SString fun_name)::_) ->
...
and sexp_to_sqlpp_const_list const_list =
  begin match const_list with
  | [] -> []
  | (SString s) :: const_list' ->
      (Dstring (char_list_of_string s)) :: (sexp_to_sqlpp_const_list const_list')
  | (SFloat f) :: const_list' ->
      (Dforeign (Obj.magic (Enhancedfloat f))) :: (sexp_to_sqlpp_const_list const_list')
  | (SInt i) :: const_list' ->
      (Dnat i) :: (sexp_to_sqlpp_const_list const_list')
  | _ ->
      raise (Qcert_Error "Not well-formed S-expr inside SQL++ const_list")
  end
and sexp_to_sqlpp_cond cond =
  begin match cond with
  | STerm ("and",[cond1;cond2]) ->
      QSQLPP.sqlpp_cond_and (sexp_to_sqlpp_cond cond1) (sexp_to_sqlpp_cond cond2)
  | STerm ("or",[cond1;cond2]) ->
      QSQLPP.sqlpp_cond_or (sexp_to_sqlpp_cond cond1) (sexp_to_sqlpp_cond cond2)
  | STerm ("not",[cond1]) ->
      QSQLPP.sqlpp_cond_not (sexp_to_sqlpp_cond cond1)
  | STerm ("equal",[expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary SPEq (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("not_equal",[expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary SPDiff (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("greater_than",[expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary SPGt (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("greater_than_or_equal",[expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary SPGe (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("less_than",[expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary SPLt (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("less_than_or_equal",[expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary SPLe (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("like",[expr1;slike]) ->
      QSQLPP.sqlpp_cond_like (sexp_to_sqlpp_expr expr1) (sstring_to_coq_string slike)
  | STerm ("exists",[query]) ->
      QSQLPP.sqlpp_cond_exists (sexp_to_sqlpp_query query)
  | STerm ("isBetween",[expr1;expr2;expr3]) ->
      QSQLPP.sqlpp_cond_between (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2) (sexp_to_sqlpp_expr expr3)
  | STerm ("isIn",[expr1;expr2]) ->
      QSQLPP.sqlpp_cond_in (sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("isNull",[expr1]) ->
      QSQLPP.sqlpp_cond_binary SPEq (sexp_to_sqlpp_expr expr1) (QSQLPP.sqlpp_expr_const (Dunit))
  | STerm ("function",[SString "date_le";expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary (SPBinaryForeignCond (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_le)))
	(sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "date_lt";expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary (SPBinaryForeignCond (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_lt)))
	(sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "date_gt";expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary (SPBinaryForeignCond (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_gt)))
	(sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "date_ge";expr1;expr2]) ->
      QSQLPP.sqlpp_cond_binary (SPBinaryForeignCond (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_ge)))
	(sexp_to_sqlpp_expr expr1) (sexp_to_sqlpp_expr expr2)
  | STerm ("function",[SString "date_between";expr1;expr2;expr3]) ->
      let sqlpp_expr1 = (sexp_to_sqlpp_expr expr1) in
      let sqlpp_expr2 = (sexp_to_sqlpp_expr expr2) in
      let sqlpp_expr3 = (sexp_to_sqlpp_expr expr3) in
      QSQLPP.sqlpp_cond_and
	(QSQLPP.sqlpp_cond_binary (SPBinaryForeignCond (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_le))) sqlpp_expr2 sqlpp_expr1)
	(QSQLPP.sqlpp_cond_binary (SPBinaryForeignCond (Obj.magic (Enhanced_binary_sql_date_op Bop_sql_date_le))) sqlpp_expr1 sqlpp_expr3)
  | STerm (sterm, _) ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ condition: " ^ sterm))
  | _ ->
      raise (Qcert_Error "Not well-formed S-expr inside SQL++ condition")
...
let rec sexp_to_sqlpp_statement (stmt : sexp)  =
  begin match stmt with
  (* Bypass for 'with' at the beginning of the query -- treated as a view *)
  | STerm ("query",((STerm ("with",[STerm ("as",[SString tname]);view_query]))::rest)) ->
      let rest_stmt = sexp_to_sqlpp_statement (STerm ("query",rest)) in
      (QSQLPP.sqlpp_create_view (Util.char_list_of_string tname) (sexp_to_sqlpp_query view_query))::rest_stmt
  | STerm ("query",_) -> [QSQLPP.sqlpp_run_query (sexp_to_sqlpp_query stmt)]
  | STerm ("createView", [SString name; query]) ->
     [QSQLPP.sqlpp_create_view (Util.char_list_of_string name) (sexp_to_sqlpp_query query)]
  | STerm ("dropView",[SString name]) -> [QSQLPP.sqlpp_drop_view (Util.char_list_of_string name)]
  | STerm (sterm, _) ->
      raise (Qcert_Error ("Not well-formed S-expr inside SQL++ statement: " ^ sterm))
  | _ ->
     raise (Qcert_Error "Not well-formed S-expr inside SQL++ statement")
...
let sexp_to_sqlpp (se : sexp) : QLang.sqlpp =
  begin match se with
  | STerm ("statements",stmts) ->
     List.concat (map sexp_to_sqlpp_statement stmts)
  | _ ->
     raise (Qcert_Error "Not well-formed S-expr in top SQL++ statements")
  end
