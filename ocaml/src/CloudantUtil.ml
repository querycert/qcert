...
let cloudant_compile_from_nra harness nrule op h =
  let mr = CompDriver.nraenv_optim_to_nnrc_optim_to_nnrcmr_comptop_optim op in
...
let add_harness harness h ((design_docs, (last_expr, last_inputs)): cloudant) : cloudant =
