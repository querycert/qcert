...
Section JSONtoData.
  ...
  Fixpoint data_to_json (d:data) : json :=
    ...
    | dbrand b (drec r) => jobject (("$class "%string, jarray (map jstring b))::(map (fun x => (fst x, data_to_json (snd x))) r))
    | dbrand b d' => jobject (("$class"%string, jarray (map jstring b))::("$data"%string, (data_to_json d'))::nil)
