...
Section TNNRCtoNNRCimp.
  ...
  Section from_stratified.
    ...
    Lemma tnnrc_expr_to_nnrc_imp_expr_some_correct {e ei} :
    ...
    Proof.
      ...
      Hint Constructors nnrc_imp_expr_type.
    ...
    Lemma nnrc_stmt_to_nnrc_imp_stmt_aux_must_assign
          {fvs x s si} :
      nnrc_stmt_to_nnrc_imp_stmt_aux fvs (Term_assign x) s = Some si ->
      nnrc_imp_stmt_must_assign si x.
    Proof.
      revert x si.
    ...
    Lemma tnnrc_stmt_to_nnrc_imp_stmt_aux_some_correct_fw {fvs term s si} :
    ...
    Proof.
      ...
      - apply terminate_type.
        apply (tnnrc_expr_to_nnrc_imp_expr_some_correct (e:=NNRCBinop b s1 s2))
        ; simpl; eauto.
      - apply terminate_type.
        apply (tnnrc_expr_to_nnrc_imp_expr_some_correct (e:=NNRCUnop u s))
        ; simpl; eauto.
      - invcs typ.
        econstructor.
        + eapply nnrc_stmt_to_nnrc_imp_stmt_aux_must_assign; eauto.
      ...
      - invcs typ.
        econstructor.
        + econstructor.
          * eapply tnnrc_expr_to_nnrc_imp_expr_some_correct; eauto.
          * specialize (IHs2 _ _ _ eqq1 _ _ H5); simpl in IHs2; eauto.
        + apply terminate_type.
          econstructor; simpl.
      ...
      - invcs typ.
        econstructor; eauto.
        apply (tnnrc_expr_to_nnrc_imp_expr_some_correct eqq0); trivial.
      - invcs typ.
        econstructor; eauto.
        + apply (tnnrc_expr_to_nnrc_imp_expr_some_correct eqq0); eauto.
        + specialize (IHs2 _ _ _ eqq1 _ _ H7); simpl in IHs2; eauto.
        + specialize (IHs3 _ _ _ eqq2 _ _ H8); simpl in IHs3; eauto.
      - apply terminate_type.
        eapply (tnnrc_expr_to_nnrc_imp_expr_some_correct (e:= (NNRCGroupBy s l s0))); simpl; eauto.
    Qed.


    Theorem tnnrc_stmt_to_nnrc_imp_some_correct_fw {Γc:tbindings} {globals s si} :
      nnrc_stmt_to_nnrc_imp globals s = Some si ->
      forall {τ:rtype},
        nnrc_type Γc nil s τ ->
        [ h, Γc ⊢ si ▷ τ ].
    Proof.
      unfold nnrc_stmt_to_nnrc_imp, nnrc_imp_type; match_option
      ; intros eqq1; invcs eqq1.
      intros τ typ.
      unfold nnrc_type in *.
      apply (tnnrc_stmt_to_nnrc_imp_stmt_aux_some_correct_fw eqq typ).
    Qed.
    ...
    Section counterexample.
      ...
      Example nnrc_stmt_with_weaker_nnrc_imp_type_example : nnrc 
        := NNRCBinop (OpNatBinary NatPlus)
                     (NNRCConst (dnat 1))
                     (NNRCConst (dnat 2)).

      Example nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example
        :=     (NNRCimpPush "x"
                            (NNRCimpBinop (OpNatBinary NatPlus) (NNRCimpConst (dnat 1))
                                          (NNRCimpConst (dnat 2)))).

      Example nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example_correct
        : nnrc_stmt_to_nnrc_imp_stmt_aux nil (Term_push "x") nnrc_stmt_with_weaker_nnrc_imp_type_example = Some nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example.
      Proof.
        vm_compute; reflexivity.
      Qed.

      Example nnrc_imp_typing_for_nnrc_with_weaker_nnrc_imp_type_example 
        : [ nil ; pd_tbindings_lift nil , (add_term_mc (Term_push "x") ⊤ nil) , (add_term_md (Term_push "x") ⊤ nil) ⊢ nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example ].
      Proof.
        unfold nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example.
        simpl.
        econstructor; simpl.
        - repeat econstructor.
        - reflexivity.
      ...
      Example nnrc_typing_for_nnrc_with_weaker_nnrc_imp_type_example 
        : nnrc_type nil nil nnrc_stmt_with_weaker_nnrc_imp_type_example Nat.
      Proof.
        unfold nnrc_type, nnrc_stmt_with_weaker_nnrc_imp_type_example.
        simpl.
        econstructor; simpl
        ; repeat econstructor.
      Qed.

      Example not_nnrc_typing_for_nnrc_with_weaker_nnrc_imp_type_example 
        : ~ nnrc_type nil nil nnrc_stmt_with_weaker_nnrc_imp_type_example ⊤.
      Proof.
        unfold nnrc_type, nnrc_stmt_with_weaker_nnrc_imp_type_example
        ; simpl; intros typ.
        invcs typ.
      ...
      Example nnrc_typing_weaker_nnrc_imp_translated_typing :
        nnrc_stmt_to_nnrc_imp_stmt_aux nil (Term_push "x")
                                       nnrc_stmt_with_weaker_nnrc_imp_type_example =
        Some nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example
        /\ 
        [ nil ; pd_tbindings_lift nil , (add_term_mc (Term_push "x") ⊤ nil) , (add_term_md (Term_push "x") ⊤ nil)
                                                                                ⊢ nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example ]
        /\ ~ nnrc_type nil nil nnrc_stmt_with_weaker_nnrc_imp_type_example ⊤.
      Proof.
        split; [ | split].
        - apply nnrc_imp_of_nnrc_stmt_with_weaker_nnrc_imp_type_example_correct.
        - apply nnrc_imp_typing_for_nnrc_with_weaker_nnrc_imp_type_example.
        - apply not_nnrc_typing_for_nnrc_with_weaker_nnrc_imp_type_example.
      Qed.

    (*
    Example nnrc_with_weaker_nnrc_imp_type_example : nnrc 
      := NNRCFor "x"%string (NNRCConst (dcoll (dnat 3:: dnat 4::dnat 5::nil)))
                 nnrc_stmt_with_weaker_nnrc_imp_type_example.
     *)

    (* Backwards is more problematic, since the type system for nnrc_imp allows for 
       free upcasts in a NNRCimpPush, whereas NNRC typing does not have
       such a mechanism (and NNRC is not Proper with respect to subtyping, since
       operators (such as (OpNatBinary NatPlus) are not.
       For example, 
       nnrc_typing_weaker_nnrc_imp_translated_typing witnesses a counter example.      *)
    End counterexample.
