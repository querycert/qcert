...
Section NNRCimp.
  ...
  Section Syntax.
    ...
    | NNRCimpLet : var -> option nnrc_imp_expr -> nnrc_imp_stmt -> nnrc_imp_stmt (**r variable declaration ([var $v := e₁?; s₂]) *)
  ...
  Section dec.
    ...
    Proof.
      ...
      decide equality;
        try solve [apply nnrc_imp_expr_eqdec | apply string_eqdec | apply option_eqdec].
    Defined.
  ...
  Section Core.
    ...
    Fixpoint nnrc_imp_stmtIsCore (s:nnrc_imp_stmt) : Prop :=
      ...
      | NNRCimpLet _ (Some e) s1 =>
        nnrc_imp_exprIsCore e /\ nnrc_imp_stmtIsCore s1
      | NNRCimpLet _ None s1 =>
        nnrc_imp_stmtIsCore s1
      | NNRCimpAssign _ e =>
