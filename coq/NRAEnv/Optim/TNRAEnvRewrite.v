...
Section TOptimEnv.
  ...
  Lemma tand_comm_arrow (q₁ q₂:cnraenv) :
    q₁ ∧ q₂ ⇒ q₂ ∧ q₁.
  Proof.
    unfold tcnraenv_rewrites_to; intros; simpl.
    intuition; cnraenv_inferer. generalize envand_comm; intros.
    unfold cnraenv_eq in H.
  ...
  Proof.
    unfold tcnraenv_rewrites_to; intros.
    cnraenv_inferer. econstructor; eauto.
  ...
  Proof.
    unfold tcnraenv_rewrites_to; intros.
    cnraenv_inferer.
    split;[cnraenv_inferer|idtac]; intros.
    input_well_typed.
    dtype_inverter.
  ...
  (* From Ullman's database book:
      ...
      Lemma selection_split_or (q q₁ q₂:cnraenv) :
 ...
 *)
  ...
  Proof.
    unfold tcnraenv_rewrites_to; intros.
    assert (exists τ, σ⟨ q₁ ⟩(σ⟨ q₂ ⟩( q )) ▷ τin >=> Coll τ ⊣ τc;τenv) by cnraenv_inferer.
    split; try assumption; intros.
    cnraenv_inferer.
    elim H0; clear H0; intros τ; intros.
    clear H τout.
    assert (σ⟨ q₂ ⟩(σ⟨ q₁ ⟩( q )) ▷ τin >=> Coll τ ⊣ τc;τenv) by cnraenv_inferer.
    assert (q ▷ τin >=> Coll τ ⊣ τc;τenv) by cnraenv_inferer.
    assert (q₁ ▷ τ >=> Bool ⊣ τc;τenv) by cnraenv_inferer.
    assert (q₂ ▷ τ >=> Bool ⊣ τc;τenv) by cnraenv_inferer.
  ...
  Proof.
    ...
    - generalize (@typed_cnraenv_yields_typed_data m τc τenv τin (Coll τout) c env x0 x dt_c dt_env dt_x p); intros.
  ...
  Proof.
    ...
    assert (q₁ ▷ τin >=> τout ⊣ τc;τenv)
           by (eapply tcnraenv_ignores_id_swap; eauto).
    econstructor; eauto.
    intros.
    input_well_typed.
    rewrite (cnraenv_ignores_id_swap q₁ H _ _ env x dout) in eout1.
    congruence.
  Qed.
  ...
  (* cnraenv_ignores_id q -> χ⟨ q₁ ⟩( q₂ ) ◯ᵉ q ⇒ χ⟨ q₁ ◯ᵉ q ⟩( q₂ ◯ᵉ q ) *)
  ...
  Proof.
    ...
    econstructor; eauto.
    econstructor; eauto.
    apply (tcnraenv_ignores_id_swap q H _ _ _ τenv' τenv H3).
  Qed.

  (* cnraenv_ignores_env q₁ -> χ⟨ q₁ ⟩( q₂ ) ◯ᵉ q ⇒ χ⟨ q₁ ⟩( q₂ ◯ᵉ q ) *)
  
  Lemma tappenv_over_map_cnraenv_ignores_env_arrow q₁ q₂:
    cnraenv_ignores_env q₁ ->
    χ⟨ q₁ ⟩( q₂ ) ◯ₑ ANID ⇒ χ⟨ q₁ ◯ₑ ANID ⟩( q₂ ◯ₑ ANID ).
  Proof.
    unfold tcnraenv_rewrites_to; simpl; intros.
    cnraenv_inferer.
    econstructor; eauto.
    - econstructor; eauto.
      econstructor; eauto.
      apply (tcnraenv_ignores_env_swap q₁ H _ _ _ _ _ H2).
    - intros.
  ...
  Proof.
    ...
    econstructor; eauto.
    econstructor; eauto.
    apply (tcnraenv_ignores_id_swap q H _ _ _ _ _ H3).
  Qed.
  ...
  Proof.
    ...
    econstructor; eauto.
    - repeat econstructor; eauto.
      apply (tcnraenv_ignores_id_swap q H _ τin τ1 τenv' τenv H3).
    - intros.
      input_well_typed; simpl.
      rewrite (cnraenv_ignores_id_swap q H _ _ env dout0 x).
      rewrite eout; simpl.
  ...
  Proof.
    ...
    intros; cnraenv_inferer.
    econstructor; eauto.
    apply (tcnraenv_ignores_env_swap p3 H _ _ _ _ _ H8).
  Qed.
  ...
  Proof.
    ...
    econstructor; eauto.
    - econstructor; eauto.
      apply (tcnraenv_ignores_id_swap q₁ H _ τ1 τin τout τenv' H8).
    - intros.
      input_well_typed; simpl.
      rewrite (cnraenv_ignores_id_swap q₁ H _ _ dout0 x dout).
      rewrite eout1; simpl; reflexivity.
  Qed.
    
  (* (cnraenv_ignores_env q₁) -> q₁ ◯ᵉ q₂ ⇒ q₁ *)
  ...
  Proof.
    ...
    cnraenv_inferer.
    assert (q₁ ▷ τin >=> τout ⊣ τc;τenv)
      by apply (tcnraenv_ignores_env_swap q₁ H _ τin τout τenv' τenv H7).
    econstructor; eauto.
    intros.
    input_well_typed.
    rewrite (cnraenv_ignores_env_swap q₁ H _ _ env dout x) in eout1.
    rewrite eout0 in eout1.
    inversion eout1.
    reflexivity.
  Qed.

  (* cnraenv_ignores_env q₁ -> (ENV ⊗ q₁) ◯ₑ q ⇒ q ⊗ q₁ *)
  ...
  Proof.
    ...
    assert (q₁ ▷ τin >=> Rec Closed τ₂0 pf2 ⊣ τc;τenv)
      by apply (tcnraenv_ignores_env_swap q₁ H _ τin (Rec Closed τ₂0 pf2) (Rec Closed τ₁0 pf1) τenv H10).
    eauto.
    econstructor; eauto.
    assert (q₁ ▷ τin >=> Rec Open τ₂0 pf2 ⊣ τc;τenv)
      by apply (tcnraenv_ignores_env_swap q₁ H _ τin (Rec Open τ₂0 pf2) (Rec Open τ₁0 pf1) τenv H10).
    eauto.
  Qed.
  ...
  Proof.
    ...
    econstructor; eauto.
    - repeat econstructor; eauto.
      apply (tcnraenv_ignores_env_swap q₁ H _ τ₁ Bool τ₂ τenv H6).
    - intros.
      input_well_typed.
      rewrite (cnraenv_ignores_env_swap q₁ H _ _ env dout x).
      rewrite eout0.
  ...
  Proof.
    ...
    intros. cnraenv_inferer.
    generalize (tcnraenv_ignores_id_swap _ ig τc).
    econstructor;
  ...
  (* cnraenv_ignores_id q₁ -> χᵉ⟨ q₁ ⟩(ID) ◯ᵉ q₂ ⇒ χ⟨ q₁ ◯ᵉ ID ⟩(q₂) *)
  ...
  Proof.
    ...
    cnraenv_inferer; econstructor; eauto.
    - repeat econstructor; eauto.
      apply (tcnraenv_ignores_id_swap q₁ H _ τin τenv0 τ₂ τenv0); assumption.
    - intros.
      input_well_typed.
      destruct dout; try reflexivity; simpl; clear eout τout H3.
      induction l; try reflexivity; simpl.
      rewrite (cnraenv_ignores_id_swap q₁ H _ _ a x a).
      destruct (brand_relation_brands ⊢ₑ q₁ @ₑ a ⊣ c;a); try reflexivity; simpl.
  ...
  Lemma  tmap_over_flatten_map (p₁ p₂ p₃: cnraenv) :
...
Hint Rewrite @tmerge_empty_record_r_arrow : tcnraenv_optim.
Hint Rewrite @tmerge_empty_record_l_arrow : tcnraenv_optim.
Hint Rewrite @tmapenv_to_env_arrow : tcnraenv_optim.
Hint Rewrite @tselect_and_arrow : tcnraenv_optim.
Hint Rewrite @tflatten_through_appenv_arrow : tcnraenv_optim.
Hint Rewrite @tflatten_mapenv_coll_arrow : tcnraenv_optim.
Hint Rewrite @tflatten_over_double_map_arrow : tcnraenv_optim.
Hint Rewrite @tflatten_over_double_map_with_either_arrow : tcnraenv_optim.
...
Hint Rewrite @tenvflatten_map_coll_arrow : tcnraenv_optim.
Hint Rewrite @tenvmap_into_id_arrow : tcnraenv_optim.
...
Hint Rewrite @tenvmap_map_compose_arrow : tcnraenv_optim.
Hint Rewrite @tenvmap_singleton_arrow : tcnraenv_optim.
...
Hint Rewrite @tenvflatten_coll_arrow : tcnraenv_optim.
Hint Rewrite @tenvflatten_nil_arrow : tcnraenv_optim.
...
Hint Rewrite @tapp_over_const_arrow : tcnraenv_optim.
Hint Rewrite @tapp_over_env_arrow : tcnraenv_optim.
Hint Rewrite @tapp_over_id_r_arrow : tcnraenv_optim.
Hint Rewrite @tapp_over_id_l_arrow : tcnraenv_optim.
Hint Rewrite @tapp_over_app_arrow : tcnraenv_optim.
Hint Rewrite @tapp_over_unop_arrow : tcnraenv_optim.
Hint Rewrite @tapp_over_map_arrow : tcnraenv_optim.
Hint Rewrite @tapp_over_select_arrow : tcnraenv_optim.
...
Hint Rewrite @tproduct_singletons_arrow : tcnraenv_optim.
Hint Rewrite @tdouble_flatten_map_coll_arrow : tcnraenv_optim.
Hint Rewrite @ttostring_dstring_arrow : tcnraenv_optim.
Hint Rewrite @ttostring_tostring_arrow : tcnraenv_optim.
Hint Rewrite @ttostring_sconcat_arrow : tcnraenv_optim.
...
Hint Rewrite @tappenv_over_env_l_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_over_env_r_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_over_appenv_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_over_app_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_over_app_ie_arrow : tcnraenv_optim.
Hint Rewrite @tcompose_selects_in_mapenv_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_flatten_mapenv_to_map_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_over_const_arrow : tcnraenv_optim.
Hint Rewrite @tflip_env1_arrow : tcnraenv_optim.
Hint Rewrite @tflip_env2_arrow : tcnraenv_optim.
Hint Rewrite @tmapenv_over_singleton_arrow : tcnraenv_optim.
Hint Rewrite @tflip_env4_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_over_binop : tcnraenv_optim.
Hint Rewrite @tflip_env6_arrow : tcnraenv_optim.
Hint Rewrite @tmapenv_to_map_arrow : tcnraenv_optim.
Hint Rewrite @tmerge_concat_to_concat_arrow : tcnraenv_optim.
Hint Rewrite @tmerge_with_concat_to_concat_arrow : tcnraenv_optim.
Hint Rewrite @tappenv_mapenv_to_map_arrow : tcnraenv_optim.

Hint Rewrite @tmap_over_nil : tcnraenv_optim.
Hint Rewrite @tselect_over_nil : tcnraenv_optim.
Hint Rewrite @tmap_over_either  : tcnraenv_optim.
Hint Rewrite @tmap_over_either_app : tcnraenv_optim.
Hint Rewrite @tselect_over_either : tcnraenv_optim.
Hint Rewrite @tselect_over_app_either : tcnraenv_optim.
Hint Rewrite @tappenv_through_either : tcnraenv_optim.
Hint Rewrite @tcount_over_map : tcnraenv_optim.
Hint Rewrite @tcount_over_flat_map_map : tcnraenv_optim.
Hint Rewrite @tcount_over_flat_map_either_nil_map : tcnraenv_optim.
Hint Rewrite @tcount_over_flat_map_either_nil_app_map : tcnraenv_optim.
Hint Rewrite @tunop_over_either : tcnraenv_optim.
Hint Rewrite @tunop_over_either_app : tcnraenv_optim.
Hint Rewrite @tflatten_flatten_map_either_nil : tcnraenv_optim.
Hint Rewrite @tcount_over_flat_map_either_nil_app_singleton : tcnraenv_optim.
(* end hide *)
