...
let print_nraenv_result fname actual_res =
  ...
  | Some res ->
      let res_string = Pretty_common.pretty_data str_formatter res; flush_str_formatter () in
...
let print_error_opt_data d =
  ...
  | Some d ->
      Format.eprintf "%a@." Pretty_common.pretty_data d
  end
...
let print_rule_result fname (actual_res : QData.qdata option) =
  ...
  | None -> Format.printf "Evaluation for file %s : [Type Error]@." fname
  | Some res ->
      let res_string = Pretty_common.pretty_data Format.str_formatter res; Format.flush_str_formatter () in
      Format.printf "Evaluation for file %s : %s@." fname res_string
