...
let inheritance_of_conf gconf =
  Type_util.inheritance_of_schema (gconf.gconf_schema)

let parse_io f =
  begin match f with
  | None -> (None,None,None)
  | Some f -> get_io_components (Some (Parse_file.parse_io_from_file f))
  end

let parse_json_component f =
  begin match f with
  | None -> None
  | Some f -> Some (Parse_string.parse_json_from_string f)
  end
...
let complete_configuration gconf =
  let _optim_config =
    ...
    | Some f ->
	let optim_json = Parse_file.parse_json_from_file f in
  ...
  let _schema =
    ...
    | Some io -> gconf.gconf_schema <- Type_util.schema_of_io_json io
    | None -> ()
    end
  in
...
let data_of_string gconf s =
  let j = Parse_string.parse_json_from_string s in
  let h = inheritance_of_conf gconf in
  build_output h j
