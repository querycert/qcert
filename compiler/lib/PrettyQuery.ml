...
let rec pretty_nra_aux p sym ff a =
  ...
  | QcertCompiler.NRAGetConstant s -> fprintf ff "Table%a%s%a" pretty_sym sym.lfloor (string_of_char_list s) pretty_sym sym.rfloor
...
let rec pretty_nraenv_aux p sym ff a =
  ...
  | QcertCompiler.NRAEnvGetConstant s -> fprintf ff "Table%a%s%a" pretty_sym sym.lfloor (string_of_char_list s) pretty_sym sym.rfloor
...
let rec pretty_nnrc_aux p sym ff n =
  match n with
  | QcertCompiler.NNRCGetConstant v -> fprintf ff "$$%s"  (string_of_char_list v)
  | QcertCompiler.NNRCVar v -> fprintf ff "$v%s"  (string_of_char_list v)
...
let rec pretty_nnrs_expr p sym ff e =
  match e with
  | QcertCompiler.NNRSGetConstant v -> fprintf ff "$$%s"  (string_of_char_list v)
  | QcertCompiler.NNRSVar v -> fprintf ff "$v%s"  (string_of_char_list v)
...
let rec pretty_nnrs_imp_expr p sym ff e =
  match e with
  | QcertCompiler.NNRSimpGetConstant v -> fprintf ff "$$%s"  (string_of_char_list v)
  | QcertCompiler.NNRSimpVar v -> fprintf ff "$v%s"  (string_of_char_list v)
...
let pretty_imp_expr pretty_data pretty_op pretty_runtime p sym ff e =
  let rec pretty_imp_expr p sym ff e =
    match e with
    | QcertCompiler.ImpExprVar v -> fprintf ff "%s"  (string_of_char_list v)
    ...
    | QcertCompiler.ImpExprError msg -> fprintf ff "error %s"  (string_of_char_list msg)
  in
...
let pretty_imp_json_data ff d =
  fprintf ff "%s"
    (string_of_char_list (QcertCompiler.EnhancedCompiler.QData.json_to_string d))
...
let pretty_imp_json_runtime p sym pretty_imp_expr ff (op, args) =
  fprintf ff "@[<hv 2>%s(@[<hv 2>%a@]@)@]"
    (string_of_char_list (QcertCompiler.string_of_json_runtime_op op))
...
let pretty_nnrcmr_job_aux sym ff mr =
  ...
  fprintf ff "@[<hv 0>input = $v%s : %s;@\n"
    (string_of_char_list mr.QcertCompiler.mr_input) input_loc;
  fprintf ff "output = $v%s : %s;@\n"
    (string_of_char_list mr.QcertCompiler.mr_output) output_loc;
...
let pretty_mr_last sym ff mr_last =
  ...
  let pretty_arg ff (x, loc) =
    match loc with
    | QcertCompiler.Vlocal ->  fprintf ff "(%s: Scalar)" (string_of_char_list x)
    | QcertCompiler.Vdistr ->  fprintf ff "(%s: Distributed)" (string_of_char_list x)
  in
...
let rec pretty_dnnrc_aux ann plug p sym ff n =
  match n with
  | QcertCompiler.DNNRCGetConstant (a, v) -> fprintf ff "%a$%s" ann a (string_of_char_list v)
  | QcertCompiler.DNNRCVar (a, v) -> fprintf ff "%a$v%s" ann a (string_of_char_list v)
...
let rec pretty_column_aux p sym ff col =
  match col with
  | QcertCompiler.CCol v -> fprintf ff "%a%s%a" pretty_sym sym.langle (string_of_char_list v) pretty_sym sym.rangle
...
let rec pretty_dataframe_aux p sym ff ds =
  match ds with
  | QcertCompiler.DSVar v -> fprintf ff "$%s" (string_of_char_list v)
  ...
  | QcertCompiler.DSExplode (s,ds) -> fprintf ff "@[explode %s @[<hv 2>from %a@] @]" (string_of_char_list s) (pretty_dataframe_aux p sym) ds
